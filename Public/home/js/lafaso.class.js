...
$.fn.extend({
	 ...
	 * 参数均可为空
	 * */
	share:function(t,u,p,c,f,callback){
		var _t = t || $('title').text(),num = !f?50:100;
		if (_t.length > num){
			_t = _t.substring(0,num) + '...';
		}
		var
		t1 = !f?encodeURI('我在@乐蜂网 看到一个很棒的商品，'):'',t2 = !f?encodeURI('我在乐蜂网看到一个很棒的商品，'):'',t3 = !f?encodeURI(' ，大家来试试哦！'):'',t4 = !f?encodeURI('心水推荐 - '):'',
		U = encodeURIComponent(u || window.location.href),
		P = encodeURIComponent(p || 'http://img1.imglefeng.com/indexfiles/logo.jpg'),
		T = encodeURIComponent(_t),
		C = encodeURIComponent(c || t || $('title').text()),
		list = {
			sina:'http://v.t.sina.com.cn/share/share.php?url='+U+'&pic='+P+'&searchPic=false&title='+t1+T+t3,
			qqt:'http://v.t.qq.com/share/share.php?url='+U+'&source='+encodeURI('乐蜂网')+'&site=www.lefeng.com'+'&pic='+P+'&searchPic=false&title='+t1+T+t3,
		...
		$(this).click(function(){window.open(list[$(this).attr('class')]);callback&&callback();return false});		
	},
	/*
	 * 倒计时
