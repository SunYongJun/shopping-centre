...
<li class="active"><a href="/newlefeng/index.php/Admin/User/index">会员管理</a></li>
<li class="active"><a href="/newlefeng/index.php/Admin/User/index">会员列表</a></li>
...
<div class="page-content">
				...
				<table id="sample-table-1" class="table table-striped table-bordered table-hover">
					...
					<tbody>
						<?php if(is_array($list)): foreach($list as $key=>$vo): ?><tr>
							...
							<td>
								<a id="seemore" href="javascript:;"><?php echo ($vo["user_email"]); ?></a>
							</td>
							<td><?php echo ($vo["user_alias"]); ?></td>
							<td class="hidden-480"><?php echo ($vo[user_sex]?'男':'女'); ?></td>
							<td><img src="/newlefeng/Public/<?php echo ($vo[user_pic]); ?>" width="30" height="30"></td>
							<td><?php echo ($vo["user_money"]); ?></td>
							<td><?php echo ($vo["user_coin"]); ?></td>
							<td><?php echo ($vo["user_visitcount"]); ?></td>
							<td><?php echo (date("Y-m-d H:i:s",$vo["user_regtime"])); ?></td>
							<td><?php echo ($vo[is_delete]?'禁用':'未禁用'); ?></td>
							<td>
								<div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
									<a class="btn btn-xs btn-info" title="修改" href="/newlefeng/index.php/Admin/User/save/id/<?php echo ($vo["id"]); ?>">
										<i class="icon-edit bigger-120"></i>
									</a>

									<a class="btn btn-xs btn-danger" del="<?php echo ($vo["id"]); ?>" title="禁用">
	...
	$(function(){
		$('a[del]').click(function(){
			...
			layer.confirm('您确定要禁用该用户吗？',function(){
				$.post('/newlefeng/index.php/Admin/User/del',id,function(data){
					if(data == 1){
						layer.msg('禁用成功！',1,function(){
							location.reload();
						});
					}else{
						layer.msg('禁用失败！');
		...
		$('#delall').click(function(){
			...
			$.post('/newlefeng/index.php/Admin/User/delall',data,function(data){
				if(data == 1){
					layer.msg('批量禁用成功！',1,function(){
						location.reload(); //自动关闭后可做一些刷新页面等操作
					});
				}else{
					layer.msg('批量禁用失败！',2,3);
				}
			})
		})
	})
