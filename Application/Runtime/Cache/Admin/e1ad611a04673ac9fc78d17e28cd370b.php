...
<li class="active"><a href="/newlefeng/index.php/Admin/ArticleType/index">我的消息</a></li>
					...
					<div class="widget-body">
						<div class="widget-main padding-8">
							<div class="slimScrollDiv" style="position: relative; overflow: scroll;overflow-x:hidden; width: auto; height: auto;max-height:320px;">
								...
								<!-- foreach遍历自己消息 -->
								<?php if(is_array($msg)): foreach($msg as $key=>$vmsg): ?><div class="profile-activity clearfix">
									<div>
										<img class="pull-left" src="/newlefeng/Public/<?php echo ($vmsg["img"]); ?>">
										<a class="user" href="javascript:;"> <?php echo ($vmsg["name"]); ?> </a>
										标题：<?php echo ($vmsg["title"]); ?>
										<a href="javascript:;" show="<?php echo ($vmsg["id"]); ?>">查看内容</a>

										<div class="time">
											<i class="icon-time bigger-110"></i>
											<?php echo (timenow($vmsg["send_time"])); ?>
											<span class="text-success" <?php if(($vmsg["is_readed"]) != "1"): ?>style="display:none"<?php endif; ?>>已读:<?php echo (date('Y-m-d H:i:s',$vmsg["read_time"])); ?></span>	
										</div>
										<div class="tab-content" style="display:none">
											内容：<?php echo ($vmsg["content"]); ?>
										</div>
									</div>

									<div class="tools action-buttons">
										<a href="javascript:;" title="删除" class="red" del="<?php echo ($vmsg["id"]); ?>">
											<i class="icon-remove bigger-125"></i>
										</a>
										<a href="javascript:;" class="red" title="已阅" read="<?php echo ($vmsg["id"]); ?>">
			...
			<div id="modal-table" class="modal fade" tabindex="-1">
				<div class="modal-dialog">
					<div class="modal-content">
						...
						<form class="form-horizontal" role="form" action="/newlefeng/index.php/Admin/Message/insert" method="post" id="addform">
							<div class="form-group" style="margin-top:20px;">
								...
								<div class="col-sm-8">
									<select class="col-xs-10 col-sm-5" id="form-field-select-1" name="receiver_id">
										<?php if(is_array($user)): foreach($user as $key=>$vuser): ?><option value="<?php echo ($vuser["id"]); ?>"><?php echo ($vuser["admin_name"]); ?></option><?php endforeach; endif; ?>
									</select>
								</div>
							</div>
...
<script>
	$(function(){
		...
		$('#addtype').click(function(){
			...
			$.post('/newlefeng/index.php/Admin/Message/insert',data,function(data){
				if(data.status == 1){
		...
		$('a[del]').click(function(){
			...
			layer.confirm('确定删除吗？',function(index){
				layer.close(index);
				$.post('/newlefeng/index.php/Admin/Message/del',id,function(data){
					if(data.status == 1){
		...
		$('a[read]').click(function(){
			...
			$.post('/newlefeng/index.php/Admin/Message/read',id,function(data){
				if(data.status == 1){
