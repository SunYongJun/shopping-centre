								...
								<form id="allform">
									<tbody>
										<?php if(is_array($comment)): foreach($comment as $key=>$vo): ?><tr>
												...
												<td>
													<span class="lbl"><?php echo ($vo["user_email"]); ?></span>
												</td>

												<td class="pinglun">
									                        <?php $_RANGE_VAR_=explode(',',"1,19");if($vo["comment_rank"]>= $_RANGE_VAR_[0] && $vo["comment_rank"]<= $_RANGE_VAR_[1]):?><div class="rank">　</div><?php endif; ?>
									                        <?php $_RANGE_VAR_=explode(',',"20,39");if($vo["comment_rank"]>= $_RANGE_VAR_[0] && $vo["comment_rank"]<= $_RANGE_VAR_[1]):?><div class="rank1">　</div><?php endif; ?>
									                        <?php $_RANGE_VAR_=explode(',',"40,59");if($vo["comment_rank"]>= $_RANGE_VAR_[0] && $vo["comment_rank"]<= $_RANGE_VAR_[1]):?><div class="rank2">　</div><?php endif; ?>
									                        <?php $_RANGE_VAR_=explode(',',"60,79");if($vo["comment_rank"]>= $_RANGE_VAR_[0] && $vo["comment_rank"]<= $_RANGE_VAR_[1]):?><div class="rank3">　</div><?php endif; ?>
									                        <?php $_RANGE_VAR_=explode(',',"80,100");if($vo["comment_rank"]>= $_RANGE_VAR_[0] && $vo["comment_rank"]<= $_RANGE_VAR_[1]):?><div class="rank4">　</div><?php endif; ?>
									                    </td>
												<td><?php echo (mb_substr($vo["comment_content"],0,20)); ?>..</td>
												<td>
													<span><?php echo (date("Y-m-d H:i:s",$vo["comment_time"])); ?></span>
												</td>
												<td>
													<?php if($vo["is_reply"] == 1): ?>是<?php else: ?><a href="/newlefeng/index.php/Admin/Comment/reply/id/<?php echo ($vo["id"]); ?>">回复</a><?php endif; ?>
												</td>
												<td>
													<span><?php echo ($vo['is_show'] ? '显示' : '不显示'); ?></span>
													<!-- <a href="/newlefeng/index.php/Admin/Comment/edit/id/<?php echo ($vo["id"]); ?>">修改</a>
												-->
											</td>
											<td>
												<a href="javascript:;" show="<?php echo ($vo["id"]); ?>"><?php echo ($vo['is_show'] ? '禁用' : '启用'); ?></a>
												|
												<a href="/newlefeng/index.php/Admin/Comment/delete/id/<?php echo ($vo["id"]); ?>">删除</a>
											</td>
...
<script>
		$(function() {
			$('a[show]').bind('click',function() {
				...
				$.post('/newlefeng/index.php/Admin/Comment/save',{id:id,show:1},function(data){
					if($th.html() == '禁用') { 
						$th.html("启用");
						$sp.html('不显示')
						 }else{ 
			...
			$('#delall').bind('click',function() {
				...
				$.post('/newlefeng/index.php/Admin/Comment/delall',data,function(data) {				
					if(data){
