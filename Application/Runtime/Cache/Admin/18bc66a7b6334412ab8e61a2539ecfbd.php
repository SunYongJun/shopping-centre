					...
					<div class="table-responsive">
						<table id="sample-table-1" class="table table-striped table-bordered table-hover">
							...
							<?php if(is_array($list)): foreach($list as $key=>$vo): ?><tr>
									...
									<td><?php echo ($vo["acti"]); ?></td>
									<td><?php echo ($vo["username"]); ?>/<?php echo (date('Y-m-d',$vo["order_time"])); ?></td>
									<td><?php echo ($vo["name"]); ?>/<?php echo ($vo["phone"]); ?></td>
									<td><?php echo ($vo["action_count"]); ?></td>
									<td class="hidden-480">
									<?php echo ($vo["status"]); ?>
									</td>
									<td>
										<div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
											...
											<?php if($vo["action_status"] == 1): ?><a class="btn btn-xs btn-success" title="点击确认发货" ordersn="<?php echo ($vo["acti"]); ?>">
												<i class="icon-ok bigger-120"></i>
											</a><?php endif; ?>
											
											<?php if($vo["action_status"] == 4): ?><a class="btn btn-xs btn-danger" title="删除订单" del="<?php echo ($vo["id"]); ?>">
												<i class="icon-trash bigger-120"></i>
											</a><?php endif; ?>
											<a class="btn btn-xs btn-warning" title="查看订单详细信息" href="/newlefeng/index.php/Admin/Order/info/acti/<?php echo ($vo["acti"]); ?>">
	...
	$(function(){
		...
		$('a[ordersn]').click(function(){
			...
			layer.confirm('是否确认发货！',function(index){
				var data = {acti:sn};
				layer.close(index);
				$.post('/newlefeng/index.php/Admin/Order/shipping',data,function(data){
					if(data.status == 1){
		...
		$('a[del]').click(function(){
			...
			layer.confirm('确定删除吗？',function(index){
				layer.close(index);
				$.post('/newlefeng/index.php/Admin/Order/del',id,function(data){
					if(data == 1){
		...
		$('#delall').click(function(){
			...
			$.post('/newlefeng/index.php/Admin/Order/delall',data,function(data){
				if(data == 1){
