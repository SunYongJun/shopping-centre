					...
					<div class="table-responsive">
						<table id="sample-table-1" class="table table-striped table-bordered table-hover">
							...
							<?php if(is_array($list)): foreach($list as $key=>$vo): ?><tr>
									...
									<td><?php echo (date('Y-m-d H:s',$vo["book_time"])); ?></td>
									<td>
										<?php if($vo["book_status"] == 1): ?><span class="text-success">已处理</span>
											<?php else: ?>
											<span class="text-danger">未处理</span><?php endif; ?>
									</td>
									<td><?php echo ($vo["admin_name"]); ?></td>
									<td><?php if(($vo["book_status"]) == "1"): echo (date('Y-m-d H:s',$vo["admin_time"])); endif; ?></td>
									<td>
										<div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
											<a class="btn btn-xs btn-info" title="确认补货" del="<?php echo ($vo["id"]); ?>">
...
<script>
	$('a[del]').click(function(){
		...
		$.post('/newlefeng/index.php/Admin/Order/addgood','id='+id,function(data){
			if(data.status == 1){
